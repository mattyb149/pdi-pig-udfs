...
public class SelfFirstURLClassLoader extends URLClassLoader {
  private static Class<?> PKG = SelfFirstURLClassLoader.class; // for i18n purposes, needed by Translator2!!

  private String name;

  public SelfFirstURLClassLoader( URL[] url, ClassLoader classLoader ) {
    super( url, classLoader );
  }

  public SelfFirstURLClassLoader( URL[] url, ClassLoader classLoader, String name ) {
  ...
  private byte[] toBytes( InputStream is ) {
    ...
    try {
      ...
      System.out.println( "Unable To ReadClass" + e.toString() );
      return null;
    }
  }
